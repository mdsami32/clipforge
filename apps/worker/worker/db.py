"""Mirrors apps/api/app/db.py.

NOTE: in a real deploy, extract these SQLAlchemy models into a shared
internal package (e.g. `packages/db-models`) imported by both the API and
worker, instead of hand-syncing two copies. Duplicated here to keep the
Phase-1 scaffold's two services independently deployable without a shared
